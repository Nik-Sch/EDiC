...
The parser discards all components except logic \glspl{IC} (id starting with `U') save for \qty{0}{\ohm} resistors.
...
\subsubsection{\glspl{RAM} and \glspl{EEPROM} clock}
...
However, for the exact netlist \gls{FPGA} implementation this is not a good way to mimick the behavior.
...
\TODO{insert waveform}

This means, that the clock inputs of the memory and \gls{EEPROM} instantiations are replaced with the corrosponding custom clock as can be seen in \cref{lst:rom_inst}.
