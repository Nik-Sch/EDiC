...
\begin{itemize}
  ...
  \item \texttt{memRamNWE} - Memory write enable (active low): Connects to the write enable input of the \gls{SRAM} and I/O.
  \item \texttt{memRamNOE} - Memory output enable (active low): Drives the bus with the value of the \gls{SRAM} or I/O depending on the memory address (\cref{sec:memory}).
  ...
  \item \texttt{busFFNOE}\footnote{Was added in the component verification and is explained in \cref{cha:eval}.} - Drive \texttt{0xff} to the bus (active low): Connects to the output enable input of the constant \texttt{0xff} driver.
\end{itemize}
