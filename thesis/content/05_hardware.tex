...
However, a lot of decisions need to be made in choosing the correct \glspl{IC}.
...
When the $\overline{\text{EN}}$ input is low, NAND gate on the right negates the CP\footnote{The internal flip-flops of the \emph{74F825} are negative edge triggered}.
...
\Cref{fig:schClk} contains the oscillator (X1) whose frequency is determined in \cref{sec:timing} and a active low reset controller (U34) which resets on power-on and can be combined with a user reset switch (SW1301).
...
The delay time is always the worst case time as specified in the datasheet\footnote{Propagation typically varies with the temperature and age of the \gls{IC} and by taking the worst case time (maximum) it is assured that no timing bugs occur due to e.g. weather changes.}.
